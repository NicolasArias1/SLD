<?php
	...
	$wsdl="http://138.100.76.170/WebServices/Servidor.php?wsdl"; //el nombre delo archivo anterior
	...
	foreach($_POST as $name => $value) {
		if(is_numeric($value) && $name != "mlmfile")
			$vars .= "$name"."="."$value".";";
	...
	/*** Crando carpeta de resultado ***/
	$date = date("dmyHis");
	
	$pfolder = $pname.$date;
	
	$resurl = "http://138.100.76.170/results/".$uid."/".$pfolder;
	...
	//Introduciendo parametros
	//$sql->SQLParameters("localhost", "sld", "root", "");
	...
	$query = "INSERT INTO sld_practices (sid, uid, ulogin, pname, pcname, date, vars, regpath, regurl, resurl)
						VALUES ('1', '$uid', '$login', '$pname', '$pcname', '$date', '$vars', '$regpath', '$regurl', '$resurl')";
	...
	if(count($result)) {
		for($i=0, $j=0; $i < count($result); $i++) {
			if($result[$i]['state'] == 'wait') {
				...
				$pcount[$result[$i]['ip']] = $result[$i]['pcount'];
				//header("Location: ../Errors/ConnectionError.htm");
				//echo "Las estaciones que pueden ejecutar esta practica estan fuera de servicio...";
				//exit;
			}//end if
		}//end for
	...
	$in = (string)$vars."@".$path."@".$regurl."@".$pname."@".$ip.";~";
	...
	//$param=array('monto'=>'kp=*.**3;ki=*.*2;kd=*.****1;fc=1*;fi=1;T=*.*1;@D:\www\SLDStation\practicas\ControlVS@null@m_controlvs;~',);
	//$param=array('monto'=>'kp=0.003', 'monto'=>'ki=0.02', 'monto'=>'kd=0.00001', 'monto'=>'fc=10', 'monto'=>'fi=1', 'monto'=>'T=0.01', 'monto'=>'mlmfile=m_controlvs', 'monto'=>'mldir=D:\Practicas\ControlVS',);
	...
	if($err == "Response not of type text/xml: text/html"){
    	...
    	//Creando consulta
		//$query = "UPDATE sld_stations SET state='wait', pcount=".($pcount-1)." WHERE ip='$ip'";
...
// Cambios para ching OJO muy importante esta parte comentada es para el paralelismo de las estaciones, si da error de comunicacion, pongo en off esa estacion y y busco donde hacer la practica si no doy error...   
// Quite esta parte como estoy utilizandi una sola esacion, ahorrarme el time de cada ves que de error de comunicacion por cualquier cosa tener que ponerla en wait manualmente..

//    while($err && $ip) {      
//    	// Display the error
//      //echo '<h2>Error</h2><pre>' . $err . '</pre>';
//      //header("Location: ../Errors/error.php?err=".$err);
//      //header("Location: ../Errors/ws.htm");
//      
//      //Creando consulta
//			//$query = "UPDATE sld_stations SET state='wait', pcount=".($pcount-1)." WHERE ip='$ip'";
//			$query = "UPDATE sld_stations SET state='off', pcount=".($pcount-1)." WHERE ip='$ip'";    
//			
//			//Ejecutando consulta
//			$sql->SQLQuery($query);
//			
...
//	    	$err = $client->getError();
//	    }//end else
//    }//end switch           //aqui terminan los cambios
			...
			if($str && $ok == 'T') {
				for($i=0, $j=0; $i < count($files); $i++) {
				  ...
				  $query = "SELECT url FROM sld_stations WHERE ip='$ip'";
					...
					//echo $ofpath;
					//$ofpath = "http://10.12.24.214/".$stpath[$practice].$fname;
					//$dfpath = $rdirpath."\\".$fname;
		  	...
		  	//Creando consulta
				...
				//Creando consulta
				$query = "UPDATE sld_practices SET edate=('$date') WHERE id=".$pid;
