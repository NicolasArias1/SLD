...
<<<<<<< Updated upstream
	...
	$wsdl="http://127.0.0.1/WebServices/Servidor.php?wsdl"; //el nombre delo archivo anterior
...
if (count($result)) {
	for ($i = 0, $j = 0; $i < count($result); $i++) {
		...
		$pcount[$result[$i]['ip']] = $result[$i]['pcount']; // Cuantas peticiones de hacer-prÃ¡ctica tiene por ip
	} //end for
...
>>>>>>> Stashed changes
	...
	$resurl = "http://127.0.0.1/results/".$uid."/".$pfolder;
...
>>>>>>> Stashed changes
	...
	if($err == "Response not of type text/xml: text/html"){				
    	...
    	//Creando consulta
		//$query = "UPDATE sld_stations SET state='off' WHERE ip='$ip'";
