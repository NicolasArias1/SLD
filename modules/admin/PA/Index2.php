			...
			<div style="border: 5px solid #D3ECFF;">
			  ...
			  if($_POST['texto2'] ==""){
			    if ($descripcionBreve != null)
					...
					echo stripslashes("Descripci�n breve de la practica");
			  }
			  else{
				echo $_POST['texto2'];
			  }
			...
			<span id="descripcion" class="editText" name="descripcion"><?php 
			  if($_POST['descripcion'] ==""){
			    if ($descripcionLarga != null)
					...
					echo "Introduzca los par�metros del regulador PID que se desea ensayar. La estructura implementada para el regulador es de la forma suma de ganancias proporcional (Kp), integral (Ki) y derivativa (Kd). La entrada de referencia (Temperatura deseada) es igual a 5 V. (15�C).
...
La duraci�n del ensayo con la opci�n Simular es instant�nea; con la opci�n Real es de aproximadamente 2 minutos.";
			    }
			 else{
			...
			<p></p>
			<hr>
			<form id="commentForm" name="practice" action="Index3.php" method="post" enctype="multipart/form-data">
				...
				<div class="content_r_data_b"></div>
							...
							<h1 class="content_l_hst1">Seleccione el archivo simulink modificado para usar en la pr�ctica</h1>
				<input type="file" name="mdl" class="required"><br>
				<input  type="hidden" name="nomPractica" value="<?php echo $_POST["nomPractica"]; ?>">  
				<script>
				...
				<button type="button" onclick="llamarPagina('<?php echo $_POST['nomPractica']; ?>','1','<?php echo $_POST['nombreEdit']; ?>','<?php echo $_POST['select1']; ?>','<?php echo $cadenaParametrosInsert; ?>','<?php echo $_POST['select3']; ?>');">Atr�s</button>
