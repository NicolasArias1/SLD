...
else
{
    ...
    if ($_flags['include_form'] && !isset($_GET['nf']))
    {
        $_url_form      = '<div style="width:100%;margin:0;text-align:center;border-bottom:1px solid #725554;color:#000000;background-color:#F2FDF3;font-size:12px;font-weight:bold;font-family:Bitstream Vera Sans,arial,sans-serif;padding:4px;">'
                        . '<form method="post" action="' . $_script_url . '">cardila.com &nbsp; &raquo; &nbsp;'
		...
		$_url_form .= "</script>";
		$_url_form .= "<script type=\"text/javascript\">";
		$_url_form .= "var pageTracker = _gat._getTracker(\"UA-6466093-3\");";
...
if ( ereg( 'youtube', $_url ) && ereg( 'watch\?', $_url ) )
{
		//if (preg_match('/\/player2\.swf\?(.*?)", "movie_player"/', $_response_body, $match))
		if( preg_match( '#"\/watch_fullscreen\?video_id=(.*?)"#is', $_response_body, $match ) )
		{
			$file = base64_encode( 'http://youtube.com/get_video?video_id=' . $match[1] );
		}
		...
		$s[] =  '#\/\/ \<\!\[(.*?)\/\/ \]\]\>#is';
		$r[] = "
		var p = new SWFObject('/player.swf','single','450','370','7');
		p.addParam('allowfullscreen','true');
		p.addVariable('file','/?q={$file}');
