...
class adLDAP {
	...
	var $_account_suffix="@uclv.edu.cu";
	var $_base_dn = "DC=uclv,DC=edu,DC=cu"; 
	
	// An array of domain controllers. Specify multiple controllers if you 
	// would like the class to balance the LDAP queries amongst multiple servers
	var $_domain_controllers = array ("10.12.1.1");
	...
	function authenticate($domain,$username,$password,$prevent_rebind=false){
		...
		if($domain != $bdn[0]) {
			...
			for($i=0; $i < count($bdn); $i++) {
				$strdn .= $bdn[$i];
				if($i <= (count($bdn)-1))
					$strdn .= ".";
			}//end for
			
			$this->_account_suffix = "@".$strdn;
	  }//end if
