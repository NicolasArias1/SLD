<?php
	...
	//$strHTML = "";
		...
		$cat= "PR�CTICAS CREADAS";//$result1[$i]['categoria'];
		...
		for($j=0; $j < $numpract; $j++) {
			...
			$strHTML .= "<li><a href= ../modules/creacionPracticas/".$result[$j]['pname']."/".$result[$j]['pname'].".php class=\"ast1\">".tildes($result[$j]['pcname'])."                 </a>".'<a href="../creacionPracticas/'.$result[$j]['pname'].'/'.$result[$j]['pname'].'.zip" class="ast3">Descargar archivo ZIP</a>'.'<a href="eliminarPractica.php?valor='.$result[$j]['pname'].'">Borrar pr�ctica</a>';
			
			}
			
		$cat= "PR�CTICAS EN EDICION";//$result1[$i]['categoria'];
		$strHTML .= "<h1 class=\"content_r_hst2\">".$cat."</h1>";
		...
		for($j=0; $j < $numpract; $j++) {
			...
			$strHTML .= "<li>".$result[$j]['pcname']."                 </a>".'<a href="../modules/admin/PA/indexCreacion.php?nombreEdit='.$result[$j]['pname'].'&tipoSistema='.$result[$j]['tipoSistema'].'&parametros='.$result[$j]['parametros'].'&tipoEjecucion='.$result[$j]['tipoEjecucion'].'" class="ast3">Editar esta pr�ctica</a></li>';
