...
function deleteUsers(id, limit, body, order, show, page) {
	...
	if(j != 0 && j > 1)
		rs = confirm('Está seguro que desea eliminar estos usuarios?');
	else if(j != 0)
		rs = confirm('Está seguro que desea eliminar este usuario?');
	else
		rs = false;
...
function deleteComments(id, rid, advuser, limit) {
	...
	if(j != 0 && j > 1)
		rs = confirm('Está seguro que desea eliminar estos comentarios?');
	else if(j != 0)
		rs = confirm('Está seguro que desea eliminar este comentario?');
	else
		rs = false;
