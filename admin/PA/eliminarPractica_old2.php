...
if(isset($_GET["valor"]))
{
	...
	ob_end_flush();
	echo 'Práctica borrada correctamente';
}

function eliminarPractica($valor){

   $directorioAeliminar = substr($valor, 2, 0); // Corto el m_ al nombre de la práctica
   ...
   $query="DELETE FROM sld_practices_data_2 WHERE pname = '$valor'";
	...
	borrarPracticaDelServidor($valor,$real);
	...
	if($practicaExistente != 0 ){
	    $directorioAeliminar = substr($real, 2, 0);
		eliminarDir("../../creacionPracticas/".$real);
...
function borrarPracticaDelServidor($valor,$real){
	$sql = new SQL();
	$sql->SQLConnection();
	...
	$resultado    = str_replace (";".$valor, "", $practicasServidor);
	$resultado    = str_replace (";".$real, "", $resultado);
...
if(!empty($_POST['campos'])) {
  ...
  ob_end_flush();
  echo 'Prácticas borradas correctamente';
...
if(!isset($_GET["valor"]) && empty($_POST['campos'])){
	...
	header ('refresh:3; url=http://138.100.76.170/admin/PA/practicas_creadas.php');
	ob_end_flush();
	echo 'No ha seleccionado ninguna práctica para borrar';
}
?>
