...
$fecha=date( "YmjHis" ); // Funcion de fecha --> Cambiar a Ymj (año/mes/dia)
$numParametros=0;

/*
Función principal, encargada de llamar a otras para la gestión de los ficheros matlab y simulink
*/
function reemplazarParams(){
	...
	// Si se trata de una práctica real o simulada 
	...
	// Login del creador de la práctica
	$ulogin = $_POST["ulogin"];
	// Selección del fichero de plantilla deseada
	...
	// Práctica nueva o en edición.
	$enEdicion=1;
	if ($tipoCreacion == 1)
		$enEdicion=0;
	
	// Definición de los parámetros para la creación de una nueva práctica
	...
	// Hay que tener en cuenta qué pasa si creo una practica y
	...
	if(($tipoCreacion == 1 || $tipoCreacion == 2) && $practicaExistente == 0){
		...
		$practicasServidor = $practicasServidor.";$nombrePractica;$nombrePractica2";
	...
	if(($tipoCreacion == 1 || $tipoCreacion == 2) && $practicaExistente != 0){
		// Se recupera la información referente a la práctica existente
		...
		if($practicaDoble == 1){
			...
			subirFicheroSimulink($texto,$nombrePractica,$directorioRaizSimulink);
			crearFicheroZip($nombrePractica,$directorioFicheroZip,$directorioRaizArchivos);
		}else{ // Si la práctica no es doble
			
			// Practica sólamente simulada: Se borra la información posible de la real existente y se crea
			if($realSimulada == 1){
				...
				$practicaTempo=substr($nombrePractica, 0, -1);
				$practicaTempo = $practicaTempo."r";
				eliminarDir("creacionPracticas/".$practicaTempo );
				
			}// Practica sólamente real: Se borra la información posible de la simulada existente y se crea
			if($realSimulada == 2){
			...
			// Se actualizan los datos de la práctica, puesto que esta ya existía	
	...
	// Se sube el fichero simulink, en caso de que no se seleccione ninguno el fichero será el último subido
	//subirFicheroSimulink($texto,$nombrePractica,$directorioRaizSimulink);
	// Se crea zip con todos los ficheros de la práctica
	//crearFicheroZip($nombrePractica,$directorioFicheroZip,$directorioRaizArchivos);
...
function actualizarPracticaExistente($nombrePractica,$nombreCortoPractica,
				...
				$realSimulada,$cadenaParametrosInsert,$tituloPractica,
				$descripcionBrevePractica,$descripcionPractica,$enEdicion,$nombreEdit){
	$query="UPDATE sld_practices_data_2 SET sid='1',pname='$nombrePractica', pcname= '$nombreCortoPractica', categoria= 'Pr&aacute;cticas del sistema t&eacute;rmico 2', purl='', path='$path', stpath='$path2', nfiles=0, type='$insertTipo', visibilidad='visible',tipoSistema=$seleccionada,tipoEsquema=1,tipoEjecucion=$realSimulada,parametros='$cadenaParametrosInsert',tituloPractica='$tituloPractica',descripcionBreve='$descripcionBrevePractica',descripcionLarga='$descripcionPractica',enEdicion=$enEdicion WHERE pname= '$nombreEdit'";
	queryEnBaseDatos($tabla,$query);
	$query="UPDATE sld_practices_data SET sid='1',pname='$nombrePractica', pcname= '$nombreCortoPractica', categoria= 'Pr&aacute;cticas del sistema t&eacute;rmico 2', purl='', path='$path', stpath='$path2', nfiles=0, type='$insertTipo', visibilidad='oculta' WHERE pname= '$nombreEdit'";
	queryEnBaseDatos($tabla,$query);

}
// Se oculta una práctica reconvertida de real <-> simulada
...
// Se crean los archivos necesarios para la práctica
...
// Se eliminan todas las referencias a una práctica: Archivos y valores de la base de datos
function eliminarPractica($practicaTempo,$directorioRaizArchivos){
	eliminarDir($directorioRaizArchivos.$practicaTempo);
	$query="DELETE FROM sld_practices_data_2 WHERE pname = '$practicaTempo'";
	queryEnBaseDatos($tabla,$query);
	$query="DELETE FROM sld_practices_data WHERE pname = '$practicaTempo'";
	queryEnBaseDatos($tabla,$query);
}


// Proceso de crear una práctica nueva, Simulada, Real o ambas
function crearNuevaPractica($practicaDoble,$nombrePractica,$file,$directorioRaizArchivos,$file_plantilla,$file_plantilla_user,$directorioRaizUsuario,$nombrePractica2,$file2,$file_plantilla2,$nombreCortoPractica,$path,$path2,$insertTipo,$seleccionada,$realSimulada,$cadenaParametrosInsert,$directorioRaizSimulink ,$tituloPractica,$descripcionBrevePractica,$descripcionPractica,$enEdicion,$ulogin,$path21,$path22){
	...
	if($practicaDoble == 1 ){
		...
		$query="INSERT INTO sld_practices_data_2 (sid, pname, pcname, categoria, path, stpath, purl, nfiles, type, visibilidad,tipoSistema,tipoEsquema,tipoEjecucion,parametros,ficheroSimulink,tituloPractica,descripcionBreve,descripcionLarga,enEdicion,ulogin) VALUES ('1', '$nombrePractica2_m', '$nombreCortoPractica', 'Pr&aacute;cticas del sistema t&eacute;rmico 2', '$path21','$path22', '', 0, 'Real', 'oculta',$seleccionada,1,$realSimulada,'$cadenaParametrosInsert','$directorioRaizSimulink','$tituloPractica','$descripcionBrevePractica','$descripcionPractica',$enEdicion,'$ulogin')";
		queryEnBaseDatos($tabla,$query);
		$query="INSERT INTO sld_practices_data (sid, pname, pcname, categoria, path, stpath, purl, nfiles, type, visibilidad) VALUES ('1', '$nombrePractica2_m', '$nombreCortoPractica', 'Pr&aacute;cticas del sistema t&eacute;rmico 2', '$path21','$path22', '', 0, 'Real', 'oculta')";
		queryEnBaseDatos($tabla,$query);
	}
}

// Se crea fichero zip con todos los archivos de la práctica
...
// Se sube un fichero simulink con el nombre recibido por parámetro
...
// Se selecciona el fichero plantilla en función del tipo de práctica que se haya seleccionado en la pantalla
// Inicial
function seleccionarPlantillas($realSimulada){
	...
	// Switch para selección de práctica: térmico, motor...
	switch ($seleccionada)
	{
		case "1": // Caso de térmicos
			if($realSimulada == 1){ // Práctica simulada
			...
			if($realSimulada == 2){ // Práctica real
...
// Se crea un fichero .m con los parámetros principales y sus valores
...
// Se crea un archivo con la ruta especificada y el contenido recibido por parámetro
...
// Se definen los parametros añadidos dinamicamente en el formato etiqueta = valorPorDefecto
