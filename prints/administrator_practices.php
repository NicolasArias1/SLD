...
<?php
		...
		<div id="mostrarB" class="col-md-6 dropdown">
			...
			<?php
									for($i=1; $i <= $this->nshow; $i++) {
										?>
										<span  id="show_element<?php echo $i; ?>" class="dropdown-item" onclick="schPreferences('index.php', '<?php echo $this->url; ?>', '<?php echo $this->order[0]; ?>', <?php echo $i*10; ?>, '')"><?php echo $i*10; ?> Registros</span >
										<?php
									}//end for
									?>
			</div>
	...
	<table class="table table-bordered table-hover tsize">
		<tr class="bg-dark">
			...
			<th class="col-sm-3" scope="col">Usuario</th>
			<th class="col-sm-5" scope="col">Práctica</th>
			<th class="col-sm-3" scope="col">Fecha de realización</th>
			<th class="col-sm-8" scope="col">Nota</th>
		...
		<tbody style="font-size:13px;">
			...
			<?php for($i=0; $i < $this->limits[1]; $i++) { ?>
				<tr id="resource_<?php echo ($i+1); ?>" onmouseover="colorOver(<?php echo ($i+1); ?>)" onmouseout="colorOut(<?php echo ($i+1); ?>)" onclick="selectResource(<?php echo ($i+1); ?>)" class="resource_box">
					...
					<td>
						<p>
							<?php echo $this->results[$i]['ulogin']; ?>
						</p>
					</td>

					<td>
						<a style="text-decoration:none;color:black;" href="details.php?res=<?php echo $this->results[$i]['uid']."/".$this->results[$i]['pname'].$this->results[$i]['date']; ?>&rid=<?php echo $this->results[$i]['id']; ?>" class="ast4" title="<?php echo $this->results[$i]['pname']; ?>"><?php echo $this->results[$i]['pcname']; ?></a>
					</td>
					...
					<td style="text-align:center;">
						<a href="details.php?res=<?php echo $this->results[$i]['uid']."/".$this->results[$i]['pname'].$this->results[$i]['date']; ?>&rid=<?php echo $this->results[$i]['id']; ?>" onclick="selectResource(<?php echo ($i+1); ?>)"><i class="fas fa-edit uedit"></i></a>
